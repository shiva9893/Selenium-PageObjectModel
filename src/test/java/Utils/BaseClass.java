...
public class BaseClass {
	...
	public static void updateChromeVersion() {
		try {
			...
			if (chromeVersion != null) {
				...
				// Get the latest compatible ChromeDriver version for your Chrome browser
				// version
	...
	private static String getChromeVersion() throws IOException {
		...
		// processBuilder.command("/Applications/Google\\
		// Chrome.app/Contents/MacOS/Google\\ Chrome", "--version");
	...
	private static void downloadAndReplaceChromeDriver(String driverVersion) throws IOException {
		...
		File destDir = new File(
				"C:\\Users\\Kumar\\eclipse-workspace\\SeleniumTutorial\\ChromeDriver\\chromedriver.exe");
	...
	public static WebDriver launchBrowser(String browserName) {
		try {
			if (browserName.equalsIgnoreCase("Chrome")) {
				...
				String path = System.getProperty("user.dir") + "\\ChromeDriver\\msedgedriver.exe";
				// Set the path for EdgeDriver
				System.setProperty("webdriver.edge.driver", path);

				// Set EdgeOptions for customization
				EdgeOptions edgeOptions = new EdgeOptions();
				edgeOptions.addArguments("--start-maximized"); // Start Edge maximized
